...
export async function ensureAlarmChannel(): Promise<void> {
  ...
  await notifee.createChannel({
    ...
    bypassDnd: true,
    vibration: true,
    // Bundled alarm tone (res/raw/alarm_fallback.wav, generated by
    // scripts/generateSounds.js) - rings even before JS is alive;
    // loopSound on the notification keeps it going.
    sound: 'alarm_fallback',
  });
}
...
function baseNotification(alarm: Alarm, isTest: boolean) {
  return {
    ...
    ios: {
      // Bundled 25s tone (iOS caps notification sounds at 30s) - added to the
      // Xcode project as a resource by scripts/generateSounds.js + pbxproj.
      sound: 'alarm_fallback.wav',
